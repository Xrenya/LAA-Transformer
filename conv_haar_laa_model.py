...
class HaarDWT2D(nn.Module):
    """
    Haar DWT with formulas:
      LL = (x00 + x01 + x10 + x11)/2
      LH = (-x00 + x01 - x10 + x11)/2
      HL = (-x00 - x01 + x10 + x11)/2
      HH = ( x00 - x01 - x10 + x11)/2
    where x00=x[0::2,0::2], x01=x[0::2,1::2], x10=x[1::2,0::2], x11=x[1::2,1::2]
    """
    ...
    def forward(self, x):
        ...
        # reshape into 2x2 blocks: [B,C,H2,2,W2,2]
...
class HaarIDWT2D(nn.Module):
    """Inverse of HaarDWT2D for subbands LL,LH,HL,HH (same definitions)."""
    def __init__(self):
        super().__init__()
